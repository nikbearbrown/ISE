...
export async function generateMetadata({
  ...
  if (doc) {
    return {
      title: `${doc.title} - NEU ISE Tools`,
      description: doc.description || `${doc.title} — tool by NEU ISE`,
    }
  }
  ...
  try {
    ...
    if (rows.length > 0) {
      return {
        title: `${rows[0].name} - NEU ISE Tools`,
        description: rows[0].description || `${rows[0].name} — tool by NEU ISE`,
      }
    }
  } catch {}

  return { title: 'Tool - NEU ISE' }
}
